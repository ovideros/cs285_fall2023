    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(
                env, actor, params['batch_size'], params['ep_len'], False
            )
            ...
            if params['do_dagger']:
                ...
                # Relabel collected observations with expert labels
                for path in paths:
                    obs = path["observation"]
                    expert_actions = expert_policy.get_action(obs)
                    path["action"] = expert_actions.squeeze().detach().cpu().numpy()
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          batch_indices = np.random.randint(0, replay_buffer.num_stored, params['train_batch_size'])
          ob_batch = replay_buffer.observations[batch_indices]
          ac_batch = replay_buffer.actions[batch_indices]
